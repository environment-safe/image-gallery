...
class GalleryImage extends _elements.HTMLElement {
  constructor() {
    super();
    this._data = [];
    this.attachShadow({
  ...
  set transitions(value) {
    const parsedValue = typeof value === 'string' ? value.split(',').map(s => s.trim()) : value;
...
_elements.customElements.define('gallery-image', GalleryImage);