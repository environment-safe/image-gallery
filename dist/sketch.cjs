...
/* global TimelineMax:false, Power2:false*/
...
class Sketch {
  constructor(opts) {
    ...
    this.container = opts.element || document.getElementById(opts.id || 'slider');
    this.images = opts.in && opts.out ? [opts.in, opts.out] : JSON.parse(this.container.getAttribute('data-images'));
  ...
  initiate(cb) {
    ...
    this.images.forEach((url, i) => {
      let promise = new Promise(resolve => {
        that.textures[i] = new _three.TextureLoader().load(url, resolve);
      });
  ...
  addObjects() {
    ...
    this.material = new _three.ShaderMaterial({
      ...
      uniforms: {
        ...
        displacement: {
          type: 'f',
          value: new _three.TextureLoader().load('img/disp1.jpg')
        },
        resolution: {
          type: 'v4',
